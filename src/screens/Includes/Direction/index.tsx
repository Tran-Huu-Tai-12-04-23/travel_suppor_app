...
import { blackColor, btnPrimary, secondaryColor, whiteColor } from '@constants/Colors';
import { useModal } from '@context/ModalContext';
import BottomSheet from '@gorhom/bottom-sheet';
import CustomHeader from '@navigation/CustomHeader';
import { localImages } from 'assets/localImage';
import React, { useState, useRef, useMemo } from 'react';
import { ScrollView, Dimensions, StyleSheet, View, Platform } from 'react-native';
...
const LATITUDE = 10.7326452;
const LONGITUDE = 106.697189;
...
const DirectionScreen: React.FC = () => {
   ...
   const [coordinates, setCoordinates] = useState<Array<LatLng>>([
      { latitude: 10.7326452, longitude: 106.697189 },
      { latitude: 10.7433902, longitude: 106.682425 },
   ]);
   ...
   const handleOpenModalChooseMap = () => {
      openModal({
         ...
         onAccept: async () => {
            await openOnMaps(coordinates[0].longitude, coordinates[0].latitude, informationDir.end_address);
            hideModal();
         },
      });
   };
   return (
      <View style={StyleSheet.absoluteFillObject}>
         ...
         >
            <Row full direction="column" style={{ padding: 10 }}>
               <Row
                  full
                  center
                  style={{ padding: 20, paddingHorizontal: 30, backgroundColor: '#DBE4E5', borderRadius: 20 }}
               >
                  <Row full direction="column">
                     <Row start full style={{ alignItems: 'center' }} colGap={10}>
                        <Icon link={localImages().pointIcon} />
                        <TextDefault bold>
                           {informationDir.start_address ? informationDir.start_address : 'Destination Place'}
                        </TextDefault>
                     </Row>
                     ...
                     <Row start full style={{ alignItems: 'center' }} colGap={10}>
                        ...
                        <TextDefault bold>
                           {informationDir.end_address ? informationDir.end_address : 'Destination Place'}
                        </TextDefault>
                     </Row>
                  </Row>
            ...
            <ScrollView style={{ flex: 1 }}>
               <Row colGap={10} between style={{ paddingVertical: 20, paddingHorizontal: 30 }}>
                  {modes.map((modeMap) => (
                     <TouchableOpacity key={modeMap.key} onPress={() => setMode(modeMap.key)}>
                        <Row
                           direction="column"
                           center
                           style={[
                              { padding: 10, borderRadius: 10, minHeight: 50, minWidth: 50 },
                              {
                                 borderStyle: 'solid',
         ...
         <Row style={[{ position: 'absolute', bottom: '40%', right: 10 }, styleGlobal.shadowForce]}>
         ...
         <CustomBottomSheet
            ...
            title={''}
         >
            <Row start direction="column" full style={{ rowGap: 10, paddingHorizontal: 20 }}>
               <Row start colGap={20}>
                  <TextDefault>Distance : </TextDefault>
                  <TextDefault bold>{informationDir.distance}</TextDefault>
               </Row>
